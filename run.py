...
Usage:
    python run.py simulate --config configs/paper_jga.json --chromosome configs/best_chromosome_jga.json
...
def cmd_simulate(args):
    ...
    if config.simulation.enable_jga:
        if pd.notna(JGA_results['v_mag_afterJGA'].values[0]):
            ...
            print("WARNING: JGA was enabled but did not trigger - the trajectory never crossed "
                  "the 5.2 AU gravity-assist distance (or integration ended at the perihelion "
                  "guard), so no assist was applied. Any reported t_200AU is a coast estimate "
                  "WITHOUT the gravity assist and will not match the paper. Check the chromosome "
                  "and config; for the shipped reference cases this should not occur.")
    else:
...
def cmd_optimize(args):
    ...
    print("  - fittest_solution_<timestamp>.xlsx")
