...
radius_jupiter = 71492e3  # m (Jupiter's equatorial radius)
YEAR = 365.25 * 24 * 3600  # Seconds in a year

DEBUG = False  # set True for verbose per-call diagnostics
...
def equations_of_motion(t, state, m_sc_0, m_EPS):
    ...
    # wrap the normalized accumulated true anomaly after a full revolution near the Sun (R < 3 AU) to keep the neurocontroller input in its trained range
    ...
    theta_thrust = np.pi * theta_thrust  # scale to radians; the output activation is confined to [-1, 0), so the commanded angle spans [-pi, 0), which allows the initial perihelion decrease
...
def PerformJupiterGravityAssist(state, JGA_Periapsis, enable_jga=True):
    ...
    if enable_jga:
        if DEBUG: print(f"Performing JGA at x={x/AU:.2f} AU, y={y/AU:.2f} AU | Before JGA: v_magnitude = {np.linalg.norm(v_spacecraft):.2f} m/s")
    else:
        if DEBUG: print(f"At Jupiter encounter (JGA disabled) at x={x/AU:.2f} AU, y={y/AU:.2f} AU | v_magnitude = {np.linalg.norm(v_spacecraft):.2f} m/s")
    ...
    if enable_jga:
        ...
        if DEBUG: print(f"  After JGA rotation: v_magnitude = {np.linalg.norm(v_final):.2f} m/s, delta_v = {np.linalg.norm(v_final) - np.linalg.norm(v_spacecraft):.2f} m/s")
        
        state[2:4] = v_final
        jga_delta_v = np.linalg.norm(v_final) - np.linalg.norm(v_spacecraft)
        energy_gain_ratio = 0.5 * (np.linalg.norm(v_final)**2 - np.linalg.norm(v_spacecraft)**2) / 5.5e9  # normalization constant for reporting only (not used in fitness)
    else:
        # JGA disabled: no velocity change
        v_final = v_spacecraft
        jga_delta_v = 0.0
        energy_gain_ratio = 0.0
        if DEBUG: print(f"  No gravity assist performed (enable_jga=False)")
...
def simulate_trajectory(individual_chromosone, fitness_settings, n_hidden_layers, n_neuronsPerLayer, activation_function, total_simulation_time_thrust_phase, rtol, atol, enable_jga=True, Isp_config=6000.0, efficiency_config=0.75):
    ...
    ## initial state ##
    delta_v = 0
    m_sc_0 = 15189 * np.exp(-0.02165 * C3)  # injected-mass model; assumption: launch with expendable Falcon Heavy (m0_max=15189 kg, gamma=0.02165 s^2/km^2)
    ...
    state0 = np.array([1 * AU, 0, vx0, vy0, m_sc_0, delta_v])

    m_EPS = (1 - mu_structure - mu_PayloadPropellantInitial) * m_sc_0  # initial Electrical Power System mass. Assumption: structural spacecraft mass is 30% of initial overall mass
    ...
    P_max = P_0 * (1 / fitness_settings['R_SolarOBerth_Design']) ** 1.5
    ...
    if DEBUG: print(f"Initial conditions - C3: {C3:.2f}, m_sc_0:{m_sc_0} launch_angle: {launch_angle/np.pi*180:.2f}, mu_PayloadPropellantInitial: {mu_PayloadPropellantInitial:.2f}, mu_EPS: {(m_EPS/m_sc_0):.2f}")
    ...
    t_span = [0, total_simulation_time_thrust_phase]
    ...
    if sol.status == 1 and np.sqrt(x[-1]**2 + y[-1]**2) >= 5.2*AU*(1 - 1e-12):  # 1 indicates a JGA event was triggered; the radius check distinguishes the 5.2 AU crossing from the perihelion guard. The event root lands on 5.2 AU only to within the root-finding tolerance (sub-millimeter), so a strict > compare made the JGA fire platform-dependently; the 1e-12 relative epsilon (~0.8 m) covers that root error with margin.
        ...
        if JGA_results['v_mag_afterJGA'][0] > np.sqrt(2 * GM_sun / np.sqrt(x[-1]**2 + y[-1]**2)):
            ...
            if DEBUG: print(f"Time to reach 200 AU: {t_200AU / (365.25 * 24 * 3600)} years")

        # Continue integration after gravity assist, only for the plot
        t_span_new = [t[-1], t[-1]+1*30*24*60*60]  # 30-day continuation for plotting
    ...
    if enable_jga or pd.notna(JGA_results['JGA_delta_v'].values[0]):
        if DEBUG: print(f"JGA results: JGA delta_v = {JGA_results['JGA_delta_v'].values[0]:.2f} m/s | v_sc after JGA = {JGA_results['v_mag_afterJGA'].values[0]:.2f} m/s  | energy gain ratio = {JGA_results['energy_gain_ratio'].values[0]:.5f}")
    if np.isfinite(t_200AU):
        if DEBUG: print(f"t200AU: {t_200AU/YEAR} years")
