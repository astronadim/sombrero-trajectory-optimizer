...
DEBUG = False  # set True for verbose per-evaluation and per-crossover diagnostics
...
def evaluate_fitness(x, y, v_mag_AfterJGA, m_payload, t_200AU, fitness_settings, P_0, enable_jga=True):
    ...
    if len(local_max_indices) == 1 and 1.2 < R_avg and 2 <= R[local_max_indices[0]] and 1 < R[-1] and P_0 > 0:
        ...
        if enable_jga:
            ...
            if R_SolarOberth_LowerBound < R_min < R_SolarOberth_UpperBound:
                ...
                if R_max < 5.2 * (1 - 1e-12):  # tolerant threshold: R_max lands on 5.2 only to within the event root-finding tolerance when integration stops at the Jupiter-distance event
                    fitness_score_ApproachJupiter = R_max / 5.2
                    fitness_score += fitness_score_ApproachJupiter
                if R_max >= 5.2 * (1 - 1e-12):
                    ...
                    if v_mag_AfterJGA >= v_escape_heliocentric_from_JupiterOrbit:  # reached solar escape velocity (heliocentric escape speed at 5.2 AU)
    ...
    if DEBUG: print(f"Total Fitness score: {fitness_score}, subFitnessScores:\n{subFitnessScores}")
...
def one_point_crossover(parent1_genome, parent2_genome, n_hidden_layers, n_neuronsPerLayer):
    ...
    if DEBUG: print(f"Performing one-point crossover at index {crossover_point}")
...
def uniform_crossover(parent1_genome, parent2_genome, n_hidden_layers, n_neuronsPerLayer):
    if DEBUG: print(f"len(parent1_genome): {len(parent1_genome)} | len(parent2_genome): {len(parent2_genome)}")
...
def node_crossover(parent1_genome, parent2_genome, n_hidden_layers, n_neuronsPerLayer):
    ...
    if DEBUG: print(f"Performing node crossover from node {start_node} [index: {start_point}] to node {end_node} [index: {end_point}]")
...
def arithmetic_crossover(parent1_genome, parent2_genome, n_hidden_layers, n_neuronsPerLayer):
    ...
    if DEBUG: print(f"genome_start_index: {genome_start_index}, input_layer_size: {input_layer_size}, hidden_layer_size: {hidden_layer_size}, output_layer_size: {output_layer_size}, total_genome_length: {total_genome_length}, len(parent1_genome): {len(parent1_genome)}")
    ...
    if DEBUG: print(f"Performing arithmetic crossover with a = {a}")
    child1_genome = np.empty_like(parent1_genome)
    child2_genome = np.empty_like(parent2_genome)
...
def mutate_genome(genome, mutation_rate, mutation_std_dev):
    ...
    for i in range(len(mutated_genome)):
        if np.random.rand() < mutation_rate:
            # initial-condition genes (0-2) get a 10x smaller mutation; use a local
            # std so the reduction does not leak into the remaining genes
            std = mutation_std_dev / 10 if i in [0, 1, 2] else mutation_std_dev
...
def tournament_selection(chromosomes, generation_fitness_scores, n_hidden_layers, n_neuronsPerLayer, mutation_rate, mutation_std_dev):
    ...
    while len(next_generation) < population_size:
        ...
        if len(next_generation) == 0:
            ...
            idx1 = fittest_index
            if DEBUG: print(f"Passing fittest individual {idx1+1} with fitness {generation_fitness_scores[fittest_index]} to the next generation")
        else:
        ...
        if fitness1 > fitness2:
            ...
            if DEBUG: print(f"Passing individual {idx1+1} to the next generation")
        else:
            fitter, unfitter = individual2, individual1
            if DEBUG: print(f"Passing individual {idx2+1} to the next generation")
        ...
        if DEBUG: print(f"Performing {crossover_method.__name__} on chromosomes {idx1+1} and {idx2+1}")
    ...
    if len(next_generation) > population_size:
        raise ValueError("The next generation has more individuals than the population size")
